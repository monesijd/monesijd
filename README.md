...
有鑑於各⼤學電資學系課程，對程式學習及成⻑脈絡⼗分重視，⽽ GitHub 平台整合了版本控制系統 Git 各項操作與記錄的介面，可以透過該平台記錄我的學習歷程與各項程式碼的演進，需要的時候可以幫助我回憶當時的思考邏輯，這對初學程式的我來說幫助很⼤。　　
...
藉由測試先行，讓我在開發函式時必須先思考介面要如何設計才容易測試，藉此幫助自己重視介面的設計，讓介面更容易被測試 (testable)。TDD 主要是分成三個步驟：

1. 先寫測試 (此時的測試**一定**會失敗)
2. 寫最少的程式，讓第一步的測試通過
3. 重構 (refactoring)

比如要開發一個 add 函式，可以加總傳入的參數：
...
不斷的重複三步驟，讓程式有對應的測試程式碼可以驗證功能正常，更重要的是幫助自己思考如何才是**可測試**的程式架構。
